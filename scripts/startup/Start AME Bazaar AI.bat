...
powershell -Command "Invoke-WebRequest -Uri http://localhost:%N8N_PORT% -TimeoutSec 2" >nul 2>&1
if %errorlevel% equ 0 (
    ...
    call :log "[INFO] Waiting for n8n to respond on http://localhost:%N8N_PORT%..."
    set "N8N_TIMEOUT=30"
    :n8n_poll
    powershell -Command "Invoke-WebRequest -Uri http://localhost:%N8N_PORT% -TimeoutSec 2" >nul 2>&1
    if %errorlevel% equ 0 (
...
:: 4. Save runtime environment variables
