...
foreach ( $why_cards as $index => $card_meta ) {
	$title = get_theme_mod( 'ame_bazaar_why_card' . $index . '_title', $card_meta['title'] );
	$desc  = get_theme_mod( 'ame_bazaar_why_card' . $index . '_desc', $card_meta['desc'] );
...
// Gracefully hide if no active cards
		...
		<div class="ame-why-choose-grid">
			<?php foreach ( $active_cards as $card ) : ?>
				<article class="ame-why-card">
					...
					<h3 class="ame-why-card-title"><?php echo esc_html( $card['title'] ); ?></h3>
					<?php if ( $card['desc'] ) : ?>
						<p class="ame-why-card-desc"><?php echo esc_html( $card['desc'] ); ?></p>
					<?php endif; ?>
				</article>
			<?php endforeach; ?>
		</div>
