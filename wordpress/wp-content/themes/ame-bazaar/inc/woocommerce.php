...
function ame_bazaar_enhance_product_schema( $markup, $product ) {
	...
	$markup['additionalProperty'][] = array(
		...
		'value' => sprintf( 'Trial available at %s %s store in Delhi', $street, $city ),
	);
...
function ame_bazaar_render_local_retail_features() {
	global $product;
	$phone    = ame_bazaar_get_business_setting( 'phone', '+91 99999 99999' );
	$whatsapp = ame_bazaar_get_business_setting( 'whatsapp' ) ?: $phone;
	$street   = ame_bazaar_get_business_setting( 'address', 'Mubarakpur Road' );

	$phone_tel_link = preg_replace( '/[^0-9+]/', '', $phone );
	$clean_wa       = preg_replace( '/[^0-9]/', '', $whatsapp );
	$whatsapp_url   = 'https://wa.me/' . $clean_wa . '?text=' . rawurlencode( 'Hi, I am interested in ' . get_the_title() );
	?>
	<div class="ame-local-retail-card" style="margin-top: 1.5rem; padding: 1.5rem; background: var(--ame-color-cream); border: 1px solid var(--ame-color-border); border-radius: var(--ame-radius-md);">
		...
		<ul class="ame-local-services-list" style="list-style:none; padding:0; margin:0 0 1.5rem 0; display:flex; flex-direction:column; gap:0.6rem; font-size:0.85rem; color:var(--ame-color-slate);">
			...
			<li style="display:flex; gap:0.4rem;"><strong>Trial room:</strong> Try before purchase at <?php echo esc_html( $street ); ?> outlet.</li>
		</ul>
		<div class="ame-local-retail-actions" style="display:flex; gap:0.8rem; flex-wrap:wrap;">
			...
			<a href="<?php echo esc_url( $whatsapp_url ); ?>" class="ame-btn-secondary" style="padding:0.6rem 1rem; font-size:0.75rem; text-decoration:none;" target="_blank" rel="noopener">
